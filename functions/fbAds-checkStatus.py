...
def lambda_handler(event, context):
    ...
    ads_queued = int(event['ads_queued'])
    ...
    slack_post_message(channel_id, token, f':tada: {message_count} ads created! :tada:')
    if ads_queued > message_count:
        slack_post_message(channel_id, token, f':warning: There are {ads_queued - message_count} ads that have not been created! :warning:')
